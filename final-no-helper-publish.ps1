...
$t0=Get-Date
$t=Join-Path $env:LOCALAPPDATA 'Temp\hlx5'
$Root2='C:\ProgramData\HomelabScreenCamera'

'=== 0. derruba tudo (helper/publisher/mediamtx/bridge) ==='
Start-Sleep -Milliseconds 500
Get-NetTCPConnection -State Listen -ErrorAction SilentlyContinue | Where-Object { $_.LocalPort -in 8000,8554,8556 } | ForEach-Object { Stop-Process -Id $_.OwningProcess -Force -ErrorAction SilentlyContinue }
Get-Process ffmpeg,python,python3.11,mediamtx -ErrorAction SilentlyContinue | Stop-Process -Force -ErrorAction SilentlyContinue
Start-Sleep -Seconds 4

'=== 1. mediamtx INTERNO 8556 + bridge 8000 + helper PUBLICO 8554->8556 ==='
...
'=== 5. leitura RTSP VIA HELPER (anon + ovifadm), watchdog 20s ==='
function TestRead($label,$url){
  $job=Start-Job -ScriptBlock { param($ff,$u) & $ff -hide_banner -loglevel error -rtsp_transport tcp -i $u -frames:v 10 -f null - 2>&1; "exit=$LASTEXITCODE" } -ArgumentList $ff,$url
  if (Wait-Job $job -Timeout 20) { "[$label] $(Receive-Job $job)" } else { Stop-Job $job -ErrorAction SilentlyContinue; "[$label] TIMEOUT(20s)" }
